...
class DecisionTree:
    ...
    def train(self, data, criteria):
        ...
        if len(cutpoints) > 1:
            ...
            if self.verbose:
                print(
                    f"branch node: {(self.parent.id if self.parent else None)} --> {self.id}"
                )
            self.subtrees = []
            ...
            if self.feature_types[self.feature_index] == "numerical":
                ...
                if self.verbose:
                    print(
                        f"using numerical feature {self.feature_index}, threshold: {self.threshold}\n"
                    )
                self.subtrees.append(
                    DecisionTree(self, verbose=self.verbose).train(
                        data[cutpoints[0] : cutpoints[1]], criteria
                    )
                )
                self.subtrees.append(
                    DecisionTree(self, verbose=self.verbose).train(
                        data[cutpoints[1] :], criteria
                    )
                )
            # For nominal features, split natrually
                ...
                if self.verbose:
                    print(f"using nominal feature {self.feature_index}\n")
                for i in range(len(cutpoints)):
                    subtree = None
                    if i == len(cutpoints) - 1:
                        subtree = DecisionTree(self, verbose=self.verbose).train(
                            data[cutpoints[i] :], criteria
                        )
                    else:
                        subtree = DecisionTree(self, verbose=self.verbose).train(
                            data[cutpoints[i] : cutpoints[i + 1]], criteria
                        )
        ...
        # No improvement achievable, make this a leave node, use majority voting to decide the label
            ...
            if self.verbose:
                print(
                    f"leaf node: {self.value}; {(self.parent.id if self.parent else None)} --> {self.id}\n"
                )
        return self
    ...
    def gain_ratio(cls, data, cutpoints):
        ...
        for i in range(len(cutpoints)):
            if i == len(cutpoints) - 1:
                intrinsic_value -= (
                    (len(data) - cutpoints[i])
                    / len(data)
                    * math.log2((len(data) - cutpoints[i]) / len(data))
                )
            else:
                intrinsic_value -= (
                    (cutpoints[i + 1] - cutpoints[i])
                    / len(data)
                    * math.log2((cutpoints[i + 1] - cutpoints[i]) / len(data))
                )
        ratio = (
            cls.information_gain(data, cutpoints) / intrinsic_value
